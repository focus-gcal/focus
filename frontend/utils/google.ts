...
class GoogleUtils {
  static generateWebAppURL(redirectURI: string, state: string) {
  ...
  static extractResponse(response: string, state: any) {
    if (!response) {
    ...
    if (!code) {
      throw new Error("No code from Google")
    }
    if (!returned_state) {
      throw new Error("No state from Google")
    }
