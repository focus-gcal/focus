...
@router.post(
    "/google/callback", response={200: OAuthCallbackOut, 400: OAuthCallbackError}
)
def google_callback(request, data: OAuthCallbackIn):
    code = data.code
    state = data.state
    redis_key = f"google_state_{state}"
    if not cache.get(redis_key):
        return 400, OAuthCallbackError(ok=False, error="State not found")
    cache.delete(redis_key)
    credentials, error = GoogleAuthService.exchange_code_for_token(code)
    if credentials is None:
        return 400, OAuthCallbackError(
    ...
    return 200, OAuthCallbackOut(ok=True, jwt_token=jwt, expiry_date=expires_at)
